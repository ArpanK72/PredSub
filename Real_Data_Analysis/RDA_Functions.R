...
# Find overlap size for estSS.
# Input  : n -> network size, k -> number of subsamples, s -> target overlap, clamp -> logical.
# Output : overlap size o.
find_o <- function(n, k, s, clamp = TRUE) {
  stopifnot(k >= 1)
  if (k == 1) {
    if (round(s) == n) return(NA_integer_)
    stop("k=1 => f(o)=n for all o; cannot hit a different nearest integer.")
  }
  m <- round((n - s) / (k - 1))
  if (clamp) m <- max(0L, min(floor(n / k), m))
  n - k * m
}
...
# Output : Xhat -> (n x d) estimated latent positions, p -> number of positive eigenvalues, t -> runtime.
